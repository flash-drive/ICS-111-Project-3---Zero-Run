...
public class PlatformerMain {
	...
	public static void main(String[] args) throws java.io.IOException {
		...
		EZSound screamSound= EZ.addSound("screamo.wav");
		EZSound music1 = EZ.addSound("guile.wav");
		EZSound music2 = EZ.addSound("music2.wav");
		music1.loop();
		...
		ArrayList<spriteCoins> coins = new ArrayList<spriteCoins>(); // arraylist for the coins being stored
		for (int i = 0; i < 10; i++) {
			coins.add(new spriteCoins("coinsprite3.png", -50, 390, 44, 44, 4)); // position of the coins
		}
		...
		while (isGameRunning) {
			...
			for (int i = 0; i < flower.size(); i++) {
				...
				if (flower.get(i).spriteSheet.isPointInElement(
					...
					EZText dead = EZ.addText(zeroX + 500, zeroY - 200, "faka wen die", Color.RED, 50);
			...
			for (int i = 0; i < Enemy.size(); i++) {
				if (Enemy.get(i).DannyPict.isPointInElement(
						Zero.spriteSheet.getXCenter() - Zero.spriteSheet.getWidth() / 2,
						Zero.spriteSheet.getYCenter() - Zero.spriteSheet.getHeight() / 2)
						|| Enemy.get(i).DannyPict.isPointInElement(
								Zero.spriteSheet.getXCenter() + Zero.spriteSheet.getWidth() / 2,
								Zero.spriteSheet.getYCenter() - Zero.spriteSheet.getHeight() / 2)
						|| Enemy.get(i).DannyPict.isPointInElement(
								Zero.spriteSheet.getXCenter() - Zero.spriteSheet.getWidth() / 2,
